#KN-  liquid 2026 04 17, 2026 05 14
#concordance of 
Sys.setenv(LANG = "en")
library(tidyverse)
library(car)
...
tumor_df <- LIQUID_DF_final%>%
  filter(!is.na(DLL1_TUMOR)) #64 cases
#tissue vs NP##########################################
##normalcy of genes ################################
#np
shapiro.test(tumor_df$NOTCH2_NP)#0.9807
shapiro.test(tumor_df$DLL1_NP)#0.1804
shapiro.test(tumor_df$HES1_NP)#0.2917
shapiro.test(tumor_df$CTNNB1_NP)#0.002585 #not normal
#tissue
shapiro.test(tumor_df$NOTCH2_TUMOR)#0.3496
shapiro.test(tumor_df$DLL1_TUMOR)#0.2464
shapiro.test(tumor_df$HES1_TUMOR)#0.2383
shapiro.test(tumor_df$CTNNB1_TUMOR)#0.05054 
#spearmann for CTNNB1

##make correltation plot####################################
#make correlation and plots between np and tissue
cor.test(tumor_df$HES1_NP, tumor_df$HES1_TUMOR, method = "pearson")#0.04355
...
#save
ggsave(
  filename = "c:/Users/Ieva/rprojects/outputs_all/LIQUID/np_tumor_blad-altman_combined2026014.png",
  plot = blad_p,
  width = 8,
  height =8,
  dpi = 300
...
ggsave(
  filename = "c:/Users/Ieva/rprojects/outputs_all/LIQUID/15_sample_pvals20260514.png",
  plot = compare_plots,
  width = 12,
  height = 8,
  dpi = 300
